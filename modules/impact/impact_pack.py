...
class FaceDetailer:
    ...
    def INPUT_TYPES(s):
        return {"required": {
                     ...
                     "cycle": ("INT", {"default": 1, "min": 1, "max": 10, "step": 1}),
                     "use_top_n": ("INT", {"default": 10, "min": 1, "max": 1000, "step": 1}),
                     "search_mask_percentage": ("FLOAT", {"default": 0.6, "min": 0.0, "max": 1.0, "step": 0.01})
                     },
                "optional": {
    ...
    def enhance_face(image, model, clip, vae, guide_size, guide_size_for_bbox, max_size, seed, steps, cfg, sampler_name, scheduler,
                     ...
                     use_top_n=10, search_mask_percentage=0.6,
                     inpaint_model=False, noise_mask_feather=0, search_mask=None):
        ...
        if search_mask is not None:
            ...
            search_mask = cv2.resize(search_mask[0].numpy(), (width, height), interpolation=cv2.INTER_NEAREST)
            search_mask[search_mask > 0.0] = 1.0
            mask_filtered_segs = []
            for mask in segs[1]:
                base_mask = np.zeros((height, width), dtype=np.float32)
                bbox = mask.bbox
                base_mask[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])] = 1.0
                area = (bbox[2] - bbox[0]) * (bbox[3] - bbox[1])
                if (base_mask * search_mask).sum() >= area * search_mask_percentage:
                    mask_filtered_segs.append(mask)
            segs = (segs[0], mask_filtered_segs)
        ordered_seggs = sorted(segs[1], key=lambda x: (x.bbox[2] - x.bbox[0]) * (x.bbox[3] - x.bbox[1]), reverse=True)[:use_top_n]
        segs = (segs[0], ordered_seggs)
        if len(segs[1]) > 0:
    ...
    def doit(self, image, model, clip, vae, guide_size, guide_size_for, max_size, seed, steps, cfg, sampler_name, scheduler,
             ...
             bbox_threshold, bbox_dilation, bbox_crop_factor,
             sam_detection_hint, sam_dilation, sam_threshold, sam_bbox_expansion, sam_mask_hint_threshold,
             sam_mask_hint_use_negative, drop_size, bbox_detector, wildcard, cycle=1, use_top_n=10, search_mask_percentage=0.6,
             sam_model_opt=None, segm_detector_opt=None, detailer_hook=None, inpaint_model=False, noise_mask_feather=0, search_mask=None):
        ...
        for i, single_image in enumerate(image):
            enhanced_img, cropped_enhanced, cropped_enhanced_alpha, mask, cnet_pil_list = FaceDetailer.enhance_face(
                ...
                cycle=cycle, use_top_n=use_top_n, search_mask_percentage=search_mask_percentage, inpaint_model=inpaint_model, noise_mask_feather=noise_mask_feather, search_mask=search_mask)
